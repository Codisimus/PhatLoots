...
public class UnidentifiedItem extends Loot {
    ...
    private static class AddUnidentifiedItemButton extends Button {
        private AddUnidentifiedItemButton(ItemStack item) {
            super(item);
        }
        ...
        public boolean onClick(Inventory inv, PhatLoot phatLoot, List<Loot> lootList) {
            instantiateTierList();
            lootList.add(new UnidentifiedItem(RANDOM_TIER));
            return true;
        }
    }

    /**
     * Registers the UnidentifiedItem button for the loot GUI
     */
    public static void registerButton() {
        //Register the Add Collection Button
        ItemStack item = new ItemStack(Material.ITEM_FRAME);
        ItemMeta info = Bukkit.getItemFactory().getItemMeta(item.getType());
        info.setDisplayName("§2Add new Unidentified Item...");
        item.setItemMeta(info);

        InventoryListener.registerButton(new AddUnidentifiedItemButton(item));
    }
